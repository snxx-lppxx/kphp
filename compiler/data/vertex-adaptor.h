...
public:
  ...
  template<Operation FromOp>
  VertexAdaptor &operator=(const VertexAdaptor<FromOp> &from) noexcept {
  ...
  VertexAdaptor &set_rl_type(RLValueType valueType) noexcept {
    impl_->rl_type = valueType;
    return *this;
  }

  VertexAdaptor &set_location(const Location &location) noexcept {
    impl_->location = location;
    return *this;
  }

  template<Operation Op2>
  VertexAdaptor &set_location(VertexAdaptor<Op2> other) noexcept {
    impl_->location = other.impl_->location;
    return *this;
  }

  VertexAdaptor &set_location_recursively(const Location &location) noexcept {
  ...
  template<Operation Op2>
  VertexAdaptor &set_location_recursively(VertexAdaptor<Op2> other) noexcept {
